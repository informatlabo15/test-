...
export class PlanificacionDetalleComponent  implements OnInit {
  ...
  initPlanificacionEstudiante() {
    ...
    return  this.fb.array(this.orderBy.transform(arr.controls, 'value.nombre'));
  }

  getEstudiantesByGrupo() {

    const grupoSelected =   this.planificacionDetalleForm.get('grupoID').value;
    const planificacionSelected = this.planificacionDetalleForm.get('planificacionID').value;
    if (grupoSelected > 0  && planificacionSelected > 0) {
      this.planificacionService.GetPlanificacionDetalleByGrupoAndPlanificacion(grupoSelected, planificacionSelected).subscribe(
        (  planificacionDetalle: PlanificacionDetalle[] ) => {
          ...
          if (this.planificacionDetalles.length > 0 ) {
            ...
            planificacionDetalle.forEach( plan => {
              ...
              //  Buscar alumnos nuevos que se han agregado despues de agregar la planificación. Si no estan los agrega a 
              //  lista de estudiantes pero inactivo.
              grupos[0]['grupoEstudiantes'].forEach(element => {
                if (element['activo'] === true) {
                 const  siExiste =  plan.planificacionEstudiante.filter( e =>  {
                    return e.estudianteID ===  element['estudianteID'];
                  })
              ...
              this.planificacionesControls.push(
                this.fb.group({
                  ...
                  planificacionEstudiante: this.fb.array(this.orderBy.transform(arr.controls, 'value.nombre'))
                })
              );
            });
          } else {
