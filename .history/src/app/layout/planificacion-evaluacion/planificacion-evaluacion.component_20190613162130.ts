...
export class PlanificacionEvaluacionComponent  implements OnInit {
    ...
    planificaciones[0]['planificacionEstudiante'].forEach(element => {
        if (element['activo'] === true) {
          ...
          arr.push(
            this.fb.group({
              estudianteID: element['estudianteID'],
              planificacionDetalleID: element['planificacionDetalleID'],
              nombre: element['estudiante'].nombre + ' ' + element['estudiante'].apellidoPat + ' ' + element['estudiante'].apellidoMat,
              evaluacionEstudiante: this.fb.array([])
            })
          );
        }
      });
      ...
      console.log(this.estudiantesControls);
  }
  ...
  addEvaluacionEstudianteById(e) {
    ...
    this.estudiantesControls.controls.forEach((estudiante: FormControl) => {
      const arrEstudiante = new FormArray([]);
      arrEstudiante.push(
        this.fb.group({
    ...
    // const arrEvaluaciones =  estudiante.get('evaluacionEstudiante')  as FormArray;
    // arrEvaluaciones.push(
    //   arrEstudiante
    // );

    estudiante.setControl('estudiantesControls', this.fb.array([]));
