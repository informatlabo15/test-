...
export class EstudiantesComponent implements OnInit {
  ...
  prepareSaveEstudiante(): Estudiante {
    ...
    const grupoEstudiosDeepCopy: GrupoEstudiante[] = formModel.grupoEstudiantes.map((grupoEstudiante: GrupoEstudiante) =>
    ...
    const saveEstudiante: Estudiante = {
      ...
      grupoID: formModel.grupoID as number,
      egresado: formModel.egresado as boolean,
      anioEgreso: formModel.anioEgreso as Date,
  ...
  addGrupo() {
    const form = this.estudianteForm.value;
    let flag = false;
    if (this.estudianteForm.get('grupoEstudiantes.grupoID').value !== '') {
      form.grupoEstudiantes.forEach(element => {
        if (element.grupoID === this.estudianteForm.get('grupoEstudiantes.grupoID').value) {
          flag = true;
        }
      });
      if (!flag) {
        ...
        this.alertify.error('No puede agregar un Grupo duplicado');
      }
    } else {
      this.alertify.error('Complete la información del Grupo');
    }
  }
